...
int main(int argc, char *argv[])
{
	...
	char result[1024];
	getcwd(result, 1024);
	cout << "cwd: " << result << endl;
	cout << "input dir: " << argv[1] << endl;
	FrameLoader iLoader(argv[1], pFrontCamera,
	...
	int nBeginIndex = std::stoi(argv[2]);
	int nEndIndex = std::stoi(argv[3]);
	cout << "Index is " << nBeginIndex << ", " << nEndIndex << endl;

	// ofstream iOutFile("./baseline/baseline.txt");
	...
	for (int nImageIndex = nBeginIndex; nImageIndex < nEndIndex; nImageIndex++)
	{
		...
		SVPair iPair = gPairs[nImageIndex];
		cout << "Bind images" << endl;
		iSurround.m_gDistortedPairs = gPairs;
		...
		ofstream iOutFile("./baseline/" + aImageIndex + "_baseline.txt");
		ofstream iOutFile2("./baseline/" + aImageIndex + "_baseline2.txt");
		...
		iOutFile << "Error in image " << nImageIndex << " is : " << nTotalError << endl;
		cout << "Error in image " << nImageIndex << " is : " << nTotalError << endl;
		...
		iOutFile2 << "Error in image " << nImageIndex << " is : " << nTotalError << endl;
		cout << "Error in image " << nImageIndex << " is : " << nTotalError << endl;
		...
		cv::imshow("current", mSurroundView);
		cv::imshow("test", mSurroundView2);
		cv::waitKey(30);
